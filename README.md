...
抓取和翻译同步会在每个翻译批次结束后保存成功结果到共享缓存。中断后，使用相同配置正常重跑可复用已保存结果；尚未完成的批次可能需要重试。`--force` 始终绕过缓存，如需复用进度请省略该参数。缓存保存失败会终止流程，不会因此重复请求模型。`--replace` 的 raw/translated 快照及强制重翻的译文文件仍只在全部翻译成功后提交。
