...
翻译配置指纹还包含规范化 API 服务地址的哈希（协议、主机、端口和路径；不包含认证信息、查询参数或片段）。切换服务地址会重新翻译；等价地址的默认端口和末尾斜杠不影响命中。缺少服务标识的旧缓存会保留但不复用，旧记录会在下次同步时重新验证，首次升级可能增加模型调用。

抓取和普通翻译同步会在每个翻译批次结束后保存成功结果到共享缓存。中断后，使用相同配置重跑时，仍需翻译的条目可复用已保存结果；尚未完成的批次可能需要重试。缓存保存失败会终止流程，不会因此重复请求模型。`--replace` 的 raw/translated 快照及强制重翻的译文文件仍只在全部翻译成功后提交。

**强制重翻不支持断点续跑。** 中断后再次带 `--force` 运行会绕过缓存，从头重新翻译，可能重复产生费用；省略 `--force` 只执行普通同步，如果旧译文仍有效，会直接跳过，既不会继续强制任务，也不会将缓存里的新译文自动替换进文件。强制重翻过程中保存的缓存可用于其他仍需翻译的条目，但不是强制任务的恢复记录。

同步时，缺少 `config_fingerprint` 的旧译文视为来源待验证，默认重新翻译；已有指纹但配置不同、原文变化或译文为空白时也会重翻。重新翻译成功前保留原有译文；完成迁移后，相同配置下可正常跳过已验证记录。首次迁移可能增加模型调用。
...
翻译缓存只有本进程自上次成功保存后生成的译文才可覆盖已有键；从旧快照读到且未修改的条目不会覆盖其他进程的修正。如果两个进程都为同一键生成新译文，以最后提交的结果为准。快照替换会在持有 raw 和 translated 两把锁后，判断可选译文文件是否存在。

默认 `JsonStore` 适合个人和小规模分析：`analysis_runs.json` 保存任务级元数据，`llm_calls.json` 保存调用级元数据。每个受管 JSON 使用同名 `.lock` advisory lock（默认最多等待 5 秒）；读取、写入、备份恢复和多文件事务会跨进程串行化，`append_json_record()` 的读取—追加—写回作为同一临界区完成。归档推文、译文和共享缓存保存时，会在同一把锁内重新读取最新数据、合并并写回，避免覆盖其他进程新增的数据；网络请求和模型调用在锁外执行。显式快照替换仍按替换语义提交。多文件事务按规范化路径顺序取锁，避免不同调用顺序造成死锁。锁文件会保留供后续运行复用，进程退出或崩溃时操作系统会自动释放实际锁。
