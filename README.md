...
### 2. 数据采集与处理
- ✅ 自动抓取指定用户的推文（排除转发和回复）
- ✅ **智能增量抓取**：`fetch_more_history.py` 智能抓取到2024年1月1日，避免重复抓取
...
├── fetch_more_history.py        # 智能增量抓取：自动抓取到2024年1月1日或最早推文
...
| **增量抓取** | `./refetch_data.sh -i` | 日常更新、Free API | 保留现有，补充新数据 | 低（10页） |
...
- 或手动运行：`python3 fetch_more_history.py`（智能抓取到2024年1月1日）
- 脚本会自动等待限流时间（15分钟）
- 或手动设置 `MAX_PAGES` ≤ 10
