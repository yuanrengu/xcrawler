...
- `fetch-more --pages` is a shared HTTP request budget for forward fetching, backward fetching, and retries. The latest incremental status is written to `{username}_fetch_status.json` with request, data-page, retry, stop-reason, `complete`, `has_more`, and per-phase completion fields. Exit statuses are `0` for complete success, `1` for failure, and `2` when safely saved data is still partial.
